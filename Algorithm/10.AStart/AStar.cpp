...
std::vector<Node*> AStar::FindPath(Node* startNode, Node* goalNode, const std::vector<std::vector<int>>& grid)
{
    ...
    // 이웃노드 탐색(열린 리스트가 비어있지 않으면 반복).
    while (!openList.empty())
    {
        // 현재 열린 리스트에서 비용(fCost)가 가장 낮은 노드 검색.
        // 최적화 가능(우선 순위 큐, 힙).
        ...
        for (const Direction& direction : directions)
        {
            ...
            neighborNode->hCost = CalculateHueristic(neighborNode, goalNode);
            neighborNode->fCost = neighborNode->gCost + neighborNode->fCost;
...
float AStar::CalculateHueristic(Node* currentNode, Node* goalNode)
{
